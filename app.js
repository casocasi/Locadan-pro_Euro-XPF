...
function refreshDepensesSummary(){
  ...
  if(dashEl) elTotal.textContent = fmtDevise(total);
}
...
// =============================
// SAUVEGARDE DANS UN DOSSIER CHOISI
// =============================
async function backupData() {
  try {
    // Demande le dossier (une seule fois, il est mémorisé)
    const dirHandle = await window.showDirectoryPicker({
        startIn: 'documents',
        mode: 'readwrite' // force la demande
    });
    const fileHandle = await dirHandle.getFileHandle('locadan_sauvegarde.json', { create: true });
    const writable = await fileHandle.createWritable();
    const exportObj = { ...data };
    const jsonStr = JSON.stringify(exportObj, null, 2);
    await writable.write(jsonStr);
    await writable.close();
    alert("✅ Sauvegarde écrite dans :\n" + fileHandle.name);
  } catch (err) {
